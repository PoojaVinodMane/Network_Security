...
include "tools/db.php"; // Include database connection
...
$profile_image = htmlspecialchars($_SESSION["profile_image"] ?? '', ENT_QUOTES, 'UTF-8');
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($_FILES['profile_image']['name']) && $_FILES['profile_image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!in_array($fileType, $allowedTypes)) {
            ...
            if (move_uploaded_file($_FILES['profile_image']['tmp_name'], $filePath)) {
                ...
                $profile_image_error = "Failed to upload profile image.";
                $error = true;
            }
        }
    }
    ...
    if (!$error) {
        ...
        $statement->execute([$first_name, $last_name, $phone, $biography, $profile_image, $email]);
        ...
        $_SESSION["profile_image"] = htmlspecialchars($profile_image, ENT_QUOTES, 'UTF-8');
